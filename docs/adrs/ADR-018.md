...
Single Page Applications (ADR-015) require client-side routing to manage navigation between different views without full page reloads. A routing solution must provide:
...
- **Framework Routers (Next.js, Remix):** Require server-side rendering, which violates our SPA architecture (ADR-015)
...
- **Integration:** Seamless integration with TanStack Query (ADR-019) for data fetching and caching.
