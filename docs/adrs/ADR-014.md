# ADR-014: No Path Aliases in TypeScript Code
...
For **Dasolve's monorepo structure** ([ADR-013](./ADR-013.md)) with multiple packages under `/packages`, path aliases would create ambiguity and maintenance burden. Relative imports and package names provide clarity and work consistently across all tools.
...
Path mapping for **package names** in the monorepo is **automatically configured by Moonrepo** ([ADR-010](./ADR-010.md)). The `@solution/*` namespace is used for all packages within the solution (e.g., `@solution/frontend`, `@solution/backend`, `@solution/datamodel-users`).
...
- **DO** rely on Moonrepo ([ADR-010](./ADR-010.md)) to handle all path mapping configuration automatically.
