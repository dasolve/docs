...
While TanStack Query (ADR-018) handles server state excellently, applications still need a solution for **global client state** that must be shared across components without prop drilling.
...
- **Server state** (use TanStack Query per ADR-018)
- **URL state** (use TanStack Router search params per ADR-017)
...
- **DON'T** use Jotai for server state. Use TanStack Query (ADR-018) instead.
- **DON'T** use Jotai for URL-based state. Use TanStack Router search params (ADR-017) instead.
